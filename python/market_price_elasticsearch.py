...
# 공공데이터 API에서 데이터를 다운로드하여 CSV로 저장
def download_market_data():
    """공공데이터 API에서 데이터를 다운로드하여 CSV로 저장"""
    try:
        print("공공데이터 API에서 데이터를 다운로드하는 중...")
        ...
        print(f"총 {len(items)}개의 시장가격 데이터를 가져왔습니다.")
        
        # DataFrame으로 변환
        df = pd.DataFrame(items)
        
        # 원본 CSV 파일 저장
        original_csv_path = 'market_price_data_original.csv'
...
def process_csv_data(csv_path):
    ...
    try:
        ...
        # 가공된 CSV 파일 저장
        processed_csv_path = 'market_price_data_processed.csv'
...
# market_price 인덱스 생성 (기존 인덱스가 있으면 삭제 후 재생성)
def create_market_price_index():
    """market_price 인덱스 생성"""
    index_name = 'market_price'
...
def main():
    """메인 함수"""
    print("공공데이터 시장가격 Elasticsearch 저장 프로그램")
    print("=" * 60)
    ...
    # 1단계: API에서 데이터 다운로드
    print("\n[1단계] API에서 데이터 다운로드")
    original_csv_path = download_market_data()
