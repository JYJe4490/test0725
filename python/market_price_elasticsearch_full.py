...
def download_all_market_data():
    """공공데이터 API에서 전체 데이터를 다운로드하여 CSV로 저장"""
    try:
        print("공공데이터 API에서 전체 데이터를 다운로드하는 중...")
        
        all_items = []
        page = 1
        page_size = 1000  # 한 번에 가져올 데이터 수를 늘림
        ...
        while True:
            ...
            try:
                ...
                if not items:
                    print(f"페이지 {page}에서 데이터가 없습니다. 종료합니다.")
                    break
                ...
                if page > 20:
                    print("20페이지 이상 데이터를 가져왔습니다. 안전을 위해 중단합니다.")
                    break
        ...
        # 원본 CSV 파일 저장
        original_csv_path = 'market_price_data_full_original.csv'
...
def process_csv_data(csv_path):
    ...
    try:
        ...
        # 가공된 CSV 파일 저장
        processed_csv_path = 'market_price_data_full_processed.csv'
...
# market_price_full 인덱스 생성 (기존 인덱스가 있으면 삭제 후 재생성)
def create_market_price_index():
    """market_price_full 인덱스 생성"""
    index_name = 'market_price_full'
...
def main():
    """메인 함수"""
    print("공공데이터 시장가격 전체 데이터 Elasticsearch 저장 프로그램")
    print("=" * 60)
    ...
    # 1단계: API에서 전체 데이터 다운로드
    print("\n[1단계] API에서 전체 데이터 다운로드")
    original_csv_path = download_all_market_data()
    ...
    print("\n[6단계] 검색 예제")
    print("\n>>> 텍스트 검색:")
    search_market_data("감귤")
    
    print("\n>>> 특정 품목 가격 정보:")
    get_price_by_product("감귤")
